...
UNWIRED_BY_DECISION: dict[str, str] = {
    ...
    "check-design-citations.py": (
        ...
        "TWO GUARD SITES, so the next reader can find them without "
        "taking this on trust: _report_bounds in "
        "check-design-citations.py, lines 187-192 as of the commit "
        "that wrote this sentence, printing 'SELECTOR CONTROL: no "
        "DESIGN.md citations found anywhere'; and main() in "
        "check-design-citation-shape.py, lines 308-310, printing "
        "'PARSED ZERO CITATIONS. The selector is broken; a green "
        "means nothing.' THE QUOTED TEXT IS THE DURABLE HALF of each "
        "pair and the line numbers are not: both moved on the branch "
        "that wrote this row, from 185-190 and 305-307 respectively. "
        "Grep the message, not the number. "
        "Measured 2026-09-08, both directions: with the register "
        "missing it exits 1 on RegisterError, and with the register "
        "restored and the backlog swept it exits 1 on the selector. "
    ...
    "check-design-citation-shape.py": (
        ...
        "CITATIONS guard in main(), lines 308-310 as of the commit "
        "that wrote this sentence, exit 1. Grep the message rather "
        "than the number; see the row above for why."
    ),
